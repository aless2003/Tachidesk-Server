...
object Anime {
    ...
    suspend fun getAnime(
        ...
        var animeEntry =
            transaction { AnimeTable.selectAll().where { AnimeTable.id eq animeId }.first() }
        ...
        return if (animeEntry[AnimeTable.initialized] && !onlineFetch) {
            ...
            transaction {
                AnimeTable.update({ AnimeTable.id eq animeId }) {
                    ...
                    if (fetchedAnime.thumbnailUrl != null &&
                        fetchedAnime.thumbnailUrl
                            .orEmpty()
                            .isNotEmpty()
                    ) {
                        it[AnimeTable.thumbnail_url] = fetchedAnime.thumbnailUrl
                    }
                }
            }
    ...
    suspend fun getAnimeThumbnail(animeId: Int): Pair<InputStream, String> {
        ...
        return getImageResponse(saveDir, fileName) {
