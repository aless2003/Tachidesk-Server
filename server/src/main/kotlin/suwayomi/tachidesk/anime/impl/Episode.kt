        ...
        if (onlineFetch == true) {
            ...
            transaction {
                EpisodeTable
                    ...
                    .orderBy(EpisodeTable.episodeIndex to DESC)
                    .map {
    ...
    private suspend fun getSourceEpisodes(animeId: Int): List<EpisodeDataClass> {
        ...
        transaction {
            episodeList.reversed().forEachIndexed { index, fetchedEpisode ->
                val episodeEntry = EpisodeTable.selectAll().where { EpisodeTable.url eq fetchedEpisode.url }.firstOrNull()
                if (episodeEntry == null) {
                    EpisodeTable.insert {
                        it[url] = fetchedEpisode.url
                        it[name] = fetchedEpisode.name
                        it[date_upload] = fetchedEpisode.dateUpload
                        it[episode_number] = fetchedEpisode.episodeNumber
        ...
        val dbEpisodeCount = transaction { EpisodeTable.selectAll().where { EpisodeTable.anime eq animeId }.count() }
        if (dbEpisodeCount > episodeCount) { // we got some clean up due
            val dbEpisodeList = transaction { EpisodeTable.selectAll().where { EpisodeTable.anime eq animeId } }
            ...
            dbEpisodeList.forEach {
                if (it[EpisodeTable.episodeIndex] >= episodeList.size ||
                    ...
                    transaction {
                        val episodeId = it[EpisodeTable.id]
                        EpisodeTable.deleteWhere { EpisodeTable.id eq episodeId }
                    }
                }
            }
        }
...
//            EpisodeTable.selectAll().where {
//                (EpisodeTable.episodeIndex eq episodeIndex) and (EpisodeTable.anime eq animeId)
//            }.first()
//        }
//        val animeEntry = transaction { MangaTable.selectAll().where { MangaTable.id eq animeId }.first() }
...
//        val episodeCount = transaction { EpisodeTable.selectAll().where { EpisodeTable.anime eq animeId }.count() }
//
//        // update page list for this episode
//        transaction {
//            pageList.forEach { page ->
//                val pageEntry = transaction { PageTable.selectAll().where { (PageTable.episode eq episodeId) and (PageTable.index eq page.index) }.firstOrNull() }
//                if (pageEntry == null) {
//                    PageTable.insert {
//                        it[index] = page.index
//                        it[url] = page.url
